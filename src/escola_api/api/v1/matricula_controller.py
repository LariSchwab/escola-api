...
@router.post("/api/matriculas", status_code=201, tags=["matriculas"])
...
def apagar_matricula(id: int, db: Session = Depends(get_db)):
    ...
    if matricula:
        ...
        db.commit()
        return
    raise HTTPException(status_code=404, detail="Matricula não encontrada com id {id}")
...
def editar_matricula(id: int, form: MatriculaEditar, db: Session = Depends(get_db)):
    ...
    if matricula:
        matricula.curso.id = form.aluno_id
        db.commit()
        db.refresh(matricula)
        return
    raise HTTPException(status_code=404, detail="Matricula não encontrada com id {id}")
